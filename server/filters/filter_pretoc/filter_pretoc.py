...
class FilterPretoc:
	def __init__(self, name_recv_exchange, name_recv_queue, name_em_queue, name_send_queue):
		...
		func_filter = {"start_prectot": lambda x: float(x) > 0.01}

		self.filter_controller = FilterController(name_recv_exchange, name_recv_queue, name_em_queue, name_send_queue, columns_names, reduced_columns, func_filter)	
